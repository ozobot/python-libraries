...
def _make_async_iter(items: list[DeviceDescription]) -> typing.AsyncIterator[tuple[DeviceDescription, object]]:
    class _Iter:
        def __init__(self):
            self._idx = 0
...
def _patch_scan_with_items(monkeypatch, items: list[DeviceDescription]) -> None:
...
def test_table_formatter_emit() -> None:
    buf = io.StringIO()
    keys = ["name", "id", "address", "product", "version", "rssi"]
    column_size = [20, 32, 17, 8, 10, 7]
    fmt = TableFormatter(buf, keys=keys, column_size=column_size)

    data = _desc().__dict__
    data["version"] = ".".join(str(v) for v in data["version"])
...
def test_table_formatter_handles_missing_fields() -> None:
    buf = io.StringIO()
    keys = ["name", "id", "address", "product", "version", "rssi"]
    column_size = [20, 32, 17, 8, 10, 7]
    fmt = TableFormatter(buf, keys=keys, column_size=column_size)
...
def test_json_formatter_omits_version_when_none() -> None:
    buf = io.StringIO()
    fmt = JsonFormatter(buf)

    async def run():
        async with fmt:
            await fmt.emit(_desc(version=None).__dict__)

    asyncio.run(run())
    parsed = json.loads(buf.getvalue())
    assert isinstance(parsed, list)
    assert len(parsed) == 1
    assert parsed[0]["version"] is None


def test_json_formatter_emits_valid_array() -> None:
    buf = io.StringIO()
    fmt = JsonFormatter(buf)

    d1 = _desc().__dict__
    d1["version"] = ".".join(str(v) for v in d1["version"])
    d2 = _desc(address="AA:BB:CC:DD:EE:FF", name="EVO-ABCDEF", rssi=-71).__dict__
    d2["version"] = ".".join(str(v) for v in d2["version"])
...
def test_json_formatter_closes_on_exception() -> None:
    buf = io.StringIO()
    fmt = JsonFormatter(buf)

    async def run():
        async with fmt:
            await fmt.emit(_desc().__dict__)
...
def test_scan_emits_devices_with_timeout(monkeypatch) -> None:
