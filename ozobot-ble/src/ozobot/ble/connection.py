...
async def _scan_devices(scanner: bleak.BleakScanner) -> typing.AsyncIterator[tuple[DeviceDescription, bleak.BLEDevice]]:
...
class Characteristic:
    ...
    def __init__(self, *, service: UUID, characteristic: UUID, client: bleak.BleakClient) -> None:
        ...
        self._client = client
        self._characteristic_handle = self._get_gatt_characteristic(client, service=service, characteristic=characteristic)

    @contextlib.asynccontextmanager
    async def open_session(self) -> typing.AsyncIterator[Session]:
        with logger.contextualize(session=str(uuid4())):
            logger.debug("Opening session", characteristic=self._characteristic_handle.uuid, service=self._characteristic_handle.service_uuid)
    ...
    def _get_gatt_characteristic(self, client: bleak.BleakClient, *, service: UUID, characteristic: UUID) -> bleak.BleakGATTCharacteristic:
...
class Session:
    def __init__(self, read_queue: asyncio.Queue[bytes], client: bleak.BleakClient, characteristic: bleak.BleakGATTCharacteristic):
        self._client = client
