...
class ProtocolSizeError(RuntimeError):
    def __init__(self, what, size, max_size):
        super().__init__(f'{what} size ({size}) exceeded maximal allowed size ({max_size}) by protocol')
...
def str2asciiz(txt: Union[str, bytes], size: Optional[int] = None) -> bytes:
    ...
    if l < size:
        ...
        raise ProtocolSizeError(txt, l, size)
...
def convert_to[T](expected_type: Type[T], value: Any) -> T:
    ...
    if issubclass(expected_type, bytes):
        value = expected_type(value, 'ascii') if isinstance(value, str) else expected_type([value])
    ...
    # If we are not bytes, then lets try to convert us directly
        ...
        value = expected_type.decode(value) if isinstance(value, int) else expected_type.parse(value)
...
def deserialize_array(member_type, data: bytes) -> Union[List, str, bytes]:
    ...
    elif (hasattr(member_type, 'py_pack') and (member_type.py_pack == 'B')) or issubclass(member_type, bytes):
        # Array of bytes is always converted to bytes -> keep it as it is
        return data
    else:
        # The rest is converted to list of type based items
        return [member_type.deserialize(data[r:r+member_type.data_width]) for r in range(0, len(data), member_type.data_width)]


def serialize_array[T](array: List[T], member_type: Type[T], size: Optional[int] = None) -> bytes:
    ...
    if issubclass(member_type, (bytes, str)):
        ...
        data = str2asciiz(
            array_str_bytes, size
        )
    else:
        # The rest is converted from list of type based items
        data = functools.reduce(operator.add, [convert_to(member_type, x).serialize() for x in array])  # type: ignore[attr-defined]
