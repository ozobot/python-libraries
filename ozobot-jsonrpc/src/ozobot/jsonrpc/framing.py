...
class _Writable(typing.Protocol):
    async def write(self, data: bytes) -> None: ...


class FrameReader[T]:
    def __init__(self, readable: _Readable, deserializer: typing.Callable[[bytes], T]) -> None:
    ...
    async def read(self) -> typing.AsyncIterator[T]:
        while True:
            data = await self._readable.read()
...
class FrameWriter[T]:
    def __init__(self, writable: _Writable, serializer: typing.Callable[[T], bytes]) -> None:
    ...
    async def write(self, message: T) -> None:
        ...
        await self._writable.write(message_encoded)
